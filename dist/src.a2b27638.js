...
*/
function render(filteredContacts) {
  var section = document.querySelector('#contacts');
  section.innerHTML = '';
  var content = filteredContacts.map(renderContact).join("");
...
function filterHandler() {
  var selectElement = document.querySelector('#filterOptions');
...
function loadCities(contacts) {
  if (contacts) {
    var select = document.querySelector('#filterOptions');
    select.innerHTML = '';
    var uniqueCities = contacts.map(function (contact) {
      return contact.address.city;
    }).filter(function (city, index, self) {
      return self.indexOf(city) === index;
    });
    uniqueCities.unshift('-- Select a city --');
    uniqueCities.forEach(function (city, i) {
      uniqueCities[i] = {
        value: i,
        name: city
      };
...
function deleteContact(id) {
  //const hasContactsAndId = contacts.length > 0 && contacts.some(contact => contact.id === id);
...
function main() {
  render(contacts);
  loadCities(contacts);
  filterHandler(); //this is causing an issue
  //deleteButtonHandler();
}
